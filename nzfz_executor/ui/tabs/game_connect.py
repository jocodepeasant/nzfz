...
class ConnectWorker(QThread):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.finished.emit(ConnectResult.fail("连接过程发生未知异常"))
...
class GameConnectTab(QWidget):
    ...
    def _on_health_check(self) -> None:
        ...
        - 其他异常状态 → 切为 ABNORMAL
        """
        result: HealthCheckResult = self._window_manager.check_health()
        logger.debug("健康检测, status=%s", result.status.value)

        if result.status == HealthStatus.HEALTHY:
            if self._state == ConnectState.ABNORMAL:
                logger.info("连接从异常恢复, 切回已连接状态")
                self._set_status(ConnectState.CONNECTED)
        elif result.status == HealthStatus.NOT_CONNECTED:
            ...
            self._status_text.setText(f"连接异常：{result.message}")
            self._set_status(ConnectState.ABNORMAL)
            logger.warning("连接异常, status=%s, message=%s", result.status.value, result.message)
