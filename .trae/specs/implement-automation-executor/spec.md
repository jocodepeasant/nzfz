...
- 实现主执行循环（事件驱动+轮询混合），支持 dry-run 模拟执行
...
### Requirement: 事件驱动+轮询混合主循环

系统 SHALL 实现事件驱动+轮询混合的主执行循环：等待状态时使用较长轮询间隔（默认2s），检测到状态变化时立即进入动作执行；动作执行过程中使用定向验证而非全屏识别。

#### Scenario: 等待波次切换
- **WHEN** 当前波次的所有动作已执行完毕
- **THEN** 系统以 2s 间隔轮询识别波次数字，直到检测到波次变化
